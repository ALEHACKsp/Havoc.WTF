...
void DrawFilledRectExA(int x, int y, int w, int h, RGBA* color)
{
	ImGui::GetOverlayDrawList()->AddRectFilled(ImVec2(x, y), ImVec2(x + w, y + h), ImGui::ColorConvertFloat4ToU32(ImVec4(color->R / 255.0, color->G / 255.0, color->B / 255.0, 0.50f)), 0, 0);
}
void cache()
{
	while (true)
	{
		...
		world = read<std::uint64_t>(m_base + 0x8AfAC50);
...
HRESULT DirectXInit(HWND hWnd)
{
	...
	io.Fonts->AddFontFromFileTTF(xorstr_("C:/Windows/Fonts/Calibri.ttf"), 15.0f);
	p_Object->Release();
	return S_OK;
}
void SetupWindow()
{
	...
	MyWnd = CreateWindowExA(NULL, xorstr_("SDK"), xorstr_("SOLUTIONS"), WS_POPUP | WS_VISIBLE, GameRect.left, GameRect.top, Width, Height, NULL, NULL, 0, NULL);
...
void render() {
	...
	ImGui::Begin("##menu", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize); {
		...
		if (Menu::show)
		{
			...
			// booleans
			static bool test;
			if (Control::RenderFOV)
			{
				DrawStrokeCircle(Width / 2, Height / 2, fovradius, &Col.black, &Col.blue, 102);
				DrawStrokeCircle(Width / 2, Height / 2, fovradius + 1, &Col.black, &Col.black, 102);
			}
			...
			std::string title = xorstr_("FORTNITE SDK.SOLUTIONS (Build: ") + builddate + xorstr_(" ") + __TIME__ + xorstr_(") (MENU IS LAGGY, USE F1, F2, F3 & F4 KEYBINDS)");

			//DrawStrokeText(1920 - 200, 20 , &Col.skyblue, xorstr_("havoc.wtf fortnite"));

			static bool kek = true;
			static int aimfovbruh;
			...
			if (cursor.y == y)
			{
				title = xorstr_("VERSION : 0.1 | SDK.SOLUTIONS");
			...
			// esp
			if (Control::DrawBoxes || Control::DrawSkeletons)
			{
				ESP();
			}

			// vk_f0x binds
			if (Menu::show && GetAsyncKeyState(VK_F1) & 1)
			{
				Control::Aimbot = !Control::Aimbot;
			}

			if (Menu::show && GetAsyncKeyState(VK_F2) & 1)
			{
				Control::RenderFOV = !Control::RenderFOV;
			}


			if (Menu::show && GetAsyncKeyState(VK_F3) & 1)
			{
				Control::DrawSkeletons = !Control::DrawSkeletons;
			}

			if (Menu::show && GetAsyncKeyState(VK_F4) & 1)
			{
				Control::DrawBoxes = !Control::DrawBoxes;
			}

			// structs
			if (Control::Aimbot)
			{
				DrawFilledRect(x + 10, y + 40, 25, 16, &Col.blue);
			}

			if (Control::RenderFOV)
			{
				DrawFilledRect(x + 10, y + 60, 25, 16, &Col.blue);
			}

			if (Control::DrawSkeletons)
			{
				DrawFilledRect(x + 10, y + 80, 25, 16, &Col.blue);
			}

			if (Control::DrawBoxes)
			{
				DrawFilledRect(x + 10, y + 100, 25, 16, &Col.blue);
			}
			...
			if (cursor.y == y + 40)
			{
				DrawFilledRectExA(x + 10, y + 40, 25, 16, &Col.blue);
			...
			if (cursor.y == y + 60)
			{
				DrawFilledRectExA(x + 10, y + 60, 25, 16, &Col.blue);
				if (GetAsyncKeyState(VK_LBUTTON) & 1)
				{
					Control::RenderFOV = !Control::RenderFOV;
				}
			...
			if (cursor.y == y + 80)
			{
				DrawFilledRectExA(x + 10, y + 80, 25, 16, &Col.blue);
				if (GetAsyncKeyState(VK_LBUTTON) & 1)
				{
					Control::DrawSkeletons = !Control::DrawSkeletons;
				}
			...
			if (cursor.y == y + 100)
			{
				DrawFilledRectExA(x + 10, y + 100, 25, 16, &Col.blue);
				if (GetAsyncKeyState(VK_LBUTTON) & 1)
				{
			...
			DrawFilledRect(cursor.x, cursor.y, 10, 10, &Col.black);



			DrawFilledRectExA(x, y, 500, 300, &Col.black);
			DrawFilledRect(x, y, 500, 30, &Col.blue);
			DrawStrokeText(x + 10, y + 7, &Col.white, title.c_str());




			DrawStrokeText(x + 30, y + 40, &Col.white, xorstr_("   Enable Aimbot // F1"));
			DrawRect(x + 10, y + 40, 25, 16, &Col.white, 1);
			DrawRect(x + 11, y + 41, 25, 16, &Col.black, 1);


			DrawStrokeText(x + 30, y + 60, &Col.white, xorstr_("   Draw FOV // F2 // UP&DOWN"));
			DrawRect(x + 10, y + 60, 25, 16, &Col.white, 1);
			DrawRect(x + 11, y + 61, 25, 16, &Col.black, 1);


			DrawStrokeText(x + 30, y + 80, &Col.white, xorstr_("   Enable ESP (skeleton) // F3"));
			DrawRect(x + 10, y + 80, 25, 16, &Col.white, 1);
			DrawRect(x + 11, y + 81, 25, 16, &Col.black, 1);


			DrawStrokeText(x + 30, y + 100, &Col.white, xorstr_("   Enable Boxes // F4"));
			DrawRect(x + 10, y + 100, 25, 16, &Col.white, 1);
			DrawRect(x + 11, y + 101, 25, 16, &Col.black, 1);
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	printf("[+] ready ! (closing this program will crash )\n", MyWnd);
	...
	printf("[+] An unexpected error occured. The program crashed and exitted the main loop. For more information please check my discord");
